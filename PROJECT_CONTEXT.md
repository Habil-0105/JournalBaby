...
- **Key behaviour:**
  ...
  - The red **Delete pill IS shown** in writing mode too (it's the current page, `isCurrent`). Unlike the carousel (which animates the removal), the writing-mode pill routes through a confirmation dialog first, then calls `store.removePage(at:)` directly — there's no carousel to play the exit animation.
...
- **Key behaviour:**
  ...
  - **Delete pill:** red "Delete" `Capsule` button at the top of the current page (`isCurrent`), shown in **both** modes. Carousel mode (`!writingMode`): tap → `store.requestDeletePage(at: pageIndex)`, the animated ghost + rising-replacement flow (§8), no confirmation. Writing mode: tap → `.alert("Delete this page?")` confirmation dialog (`@State showDeleteConfirmation`) → on confirm `store.removePage(at: pageIndex)` directly (no carousel to animate the exit, so a confirm dialog guards against accidental loss). The pill counter-scales by `1/contentScale` so it stays readable on the downscaled carousel paper.
  - **Tap handling:**
...
Deleting (current page only — Delete pill is only shown when isCurrent):

  WRITING MODE (no carousel to animate):
    PageContentView Delete pill tap (top of current page)
    ─► if store.writingMode → showDeleteConfirmation = true
...
13. **Page deletion is destructive.** A red "Delete" pill sits at the top of the current page in **both** modes. In carousel mode it runs the animated flow (`requestDeletePage` → `confirmPendingDeletion`, which preserves the exact `removePage` semantics) with no confirmation. In writing mode there is no carousel animation, so the pill opens a confirmation dialog and only then calls `store.removePage(at:)` directly. Either way the store guarantees "at least one page" via its existing fallback.
...
| Delete page            | Tap red "Delete" pill on current `PageContentView`. Carousel: `store.requestDeletePage(at:)` → animated ghost + entry via `PageCarouselView` → `store.confirmPendingDeletion()` → `store.removePage(at:)`. Writing: confirmation `.alert` → `store.removePage(at:)` directly |
...
- **Carousel deletion has no confirmation.** The red "Delete" pill in **carousel mode** calls `store.requestDeletePage(at:)` which (after the exit animation completes) eventually calls `store.removePage(at:)` directly — a single tap removes a page (and all its elements) with no chance to cancel mid-flight. Writing mode is the confirmed path (it shows a confirmation dialog because there's no animation preview); carousel relies on the visible ghost animation as feedback instead.
...
- Page deletion is animated in the carousel via `requestDeletePage(at:)` (grabs `store.pendingDeletion`) → carousel ghost + rising-replacement animation → `confirmPendingDeletion()` → `removePage(at:)`; in writing mode the Delete pill confirms via dialog and calls `removePage(at:)` directly.
...
- Whether the carousel should also confirm page deletion (currently only writing mode confirms; carousel is unconfirmed but animated).
