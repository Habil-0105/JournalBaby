...
- The app has **two visual modes** that the user switches between:
  ...
  - **Writing mode** (zoom in): `WritingCanvasView` shows just the current page as a large centered writing canvas (≈ 85% of container width, ≈ 75% of height at the paper aspect ratio, with generous margins on every side). The page strip and all neighbours are completely hidden. Elements become interactive. Enter by tapping any toolbar tool (Scribble / Add Text / Add Image / Add Audio) or by pinching in (fingers spreading); exit by pinching out (fingers closing) or tapping "Exit Writing". **Drawing is opt-in**: entering writing mode does *not* activate the pencil — the `PKCanvasView` only becomes interactive and shows the system tool picker after the user taps the toolbar Scribble tool (unless they entered via that same Scribble button, which arms it).
- **Interaction model.** Carousel mode is **browse only** — pages are preview-only, elements (text / image / audio) are non-interactive, no writing. The only live gestures are the carousel swipe, the pinch-in, and the toolbar. Writing mode is **fully interactive** — the current page's elements can be tapped, dragged, resized, edited, and the `PKCanvasView` accepts scribble strokes. The element `ForEach` in `PageContentView` is gated by `.allowsHitTesting(isCurrent && store.writingMode && !store.drawMode)`, which is the single source of truth for the mode boundary — and while Scribble is armed the elements go inert so strokes paint over them instead of dragging them.
- Three block kinds: **text** (auto‑growing notes), **image** (from the Photos library), **audio** (microphone recordings).
- A **Scribble / Draw mode** that drops the user into Apple PencilKit's native `PKCanvasView` with the system `PKToolPicker`, identical to what Notes and Freeform ship. Only active in writing mode **and only after the user arms it** via the toolbar Scribble tool (`store.drawMode`).
...
│   │       │   └── ScribbleCanvasView.swift   UIViewRepresentable wrapping PKCanvasView (interactive only when drawing is armed in writing mode)
│   │       └── Elements/
...
MiroCloneApp (@main)
   └─► JournalView            (top-level SwiftUI screen, owns CanvasStore; swaps carousel ↔ writing)
         ...
         │       WritingCanvasView    (large centered current page; PKCanvasView interactive + tool picker only when Scribble armed; pinch-out to exit)
         │         └─► PageContentView
         │               └─► ScribbleCanvasView  (interactive only when store.drawMode)
...
- `CanvasStore` (`@Published` `pages`, `currentPageIndex`, `writingMode`, `drawMode`, plus UI state `selectedElementID`, `focusedTextID`, `canvasSize`) is the **single source of truth**. `writingMode` is the mode switch: when `true`, the app shows the large centered writing canvas (only the current page); when `false`, the carousel preview (small current page with side pages deep below). `drawMode` means "Scribble / drawing is engaged": it gates the PencilKit surface (strokes + tool picker) inside writing mode. It is **not** set on entry — `enterWritingMode()` leaves it `false`, drawing is armed only by the explicit toolbar Scribble tool (`enableDrawing()`), and `select` / `focusText` / `disableDrawing` / `exitWritingMode` clear it. It also guards the carousel swipe and the page background tap.
...
- All mutations go *into* the store via methods (`addText`, `addImage(data:)`, `addAudio(fileURL:duration:)`, `moveElement`, `setWidth`, `setHeight`, `setTextHeight`, `updateElementText`, `select`, `focusText`, `clearTextFocus`, `enterWritingMode`, `exitWritingMode`, `enableDrawing`, `disableDrawing`, `remove`, `updateCanvasSize`, `addPage`, `switchToPage(at:)`, `removePage(at:)`).
...
3. `NavigationStack` + `PageCarouselView` lay out. `PageCarouselView` reports the canonical canvas size (`DesignSystem.writingCanvasSize`) to `store.updateCanvasSize`.
...
- **Shape:** `id: UUID`, `elements: [CanvasElement]`, `scribble: PKDrawing`, `createdAt: Date`. Defaults to empty; `createdAt` defaults to `Date()` and is printed at the top-center of the paper in `dd MMMM yyyy` format, so each sheet shows its own creation date.
...
- **Key behaviour:**
  - **Single canonical coordinate space.** Every page's content is authored and rendered in ONE coordinate system: `DesignSystem.writingCanvasSize` — the paper size writing mode renders 1:1 (width = min(0.85 × containerW, 0.75 × containerH / 1.3), height = width × 1.3). This same size is always reported to `store.updateCanvasSize`, so element clamps never change between modes and nothing authored on the (larger, native-resolution) writing paper can fall outside the carousel paper. The carousel **display** size is a separate, smaller `DesignSystem.carouselSlotSize` (width = min(0.45 × containerW, 0.7 × containerH / 1.3)) used only for layout/spacing; each slot renders the page content at the canonical writing size and uniformly `.scaleEffect`s it down to the slot, so both modes show the identical region of the board. The delete pill counter‑scales (`1/contentScale`) so it stays readable inside a downscaled paper.
...
- **Layer constants (private):** `spacingFactor = 1.15`, `swipeThreshold = 0.25`, `neighborVisibleFraction = 0.2`, `sidePageScale = 0.05`, `transitionResponse = 0.32`, `transitionDamping = 0.62`, `deleteAnimationDuration = 0.5`, `exitHorizontalDistanceFraction = 0.35`, `exitVerticalDropFraction = 0.45`, `exitShrinkAmount = 0.1`, `enterWritingPinchThreshold = 1.3`. (Paper sizes moved to `DesignSystem`: `pageAspectRatio = 1.3`, `writingCanvasSize(for:)`, `carouselSlotSize(for:)`.)
...
- **Key behaviour:**
  - Computes `pageSize` via the shared `DesignSystem.writingCanvasSize` — this is the **canonical content coordinate space** of a page. Significantly larger than the carousel's slot size (which uses `0.45 × containerW`) but with generous margins on all sides — never edge‑to‑edge.
  - Renders a single `PageContentView` (with `isCurrent = true`) centered in the container. The page strip is **not** rendered here.
  - The `PageContentView`'s `ScribbleCanvasView` is interactive only when drawing is engaged (`store.writingMode && store.drawMode`) — the `PKCanvasView` becomes first responder and the system `PKToolPicker` is shown. On entry writing mode is armed with drawing **off**, so neither happens until the user taps the Scribble toolbar tool.
  - The red **Delete pill IS shown in writing mode** too (it's the current page, `isCurrent`). Like the carousel, it opens the confirmation dialog first; on confirm it calls `store.removePage(at:)` directly (no carousel to animate the exit) and then `store.exitWritingMode()` — deleting from writing mode always drops the user back onto the carousel, never leaving them on a removed page.
...
- **Purpose:** Renders one page: background, scribble layer (interactive or static), a printed date label, elements, and a delete pill on the current page (`isCurrent`).
- **Key behaviour:**
  - Stack (bottom → top): page background → scribble layer → date label → elements → optional delete pill.
  - **Date label:** `page.createdAt.formatted(.dateTime.day(.twoDigits).month(.wide).year())` ("dd MMMM yyyy", e.g. "11 August 2026"), `.caption.weight(.medium)` + `.secondary`, centered at the top of the paper, `.allowsHitTesting(false)`. Shown on every paper (current page and carousel neighbors alike) and scales down with the page content in the carousel.
  - **Scribble layer branches on `isCurrent`:**
    - `isCurrent == true` → `ScribbleCanvasView(store:)` (interactive or non‑interactive `PKCanvasView` depending on `store.writingMode && store.drawMode`).
  ...
  - Element layer: `ForEach(page.elements)` renders each `ElementContainerView` at `element.position`. Hit‑testing is gated by `.allowsHitTesting(isCurrent && store.writingMode && !store.drawMode)` — neighbouring pages are never interactive, **in carousel mode (`!writingMode`) elements are non-interactive even on the current page**, **and in writing mode with Scribble armed (`drawMode`) they go inert too** so a stroke over an element paints instead of dragging it. Only writing mode with drawing off makes the current page's elements fully interactive (tap, double-tap, drag, resize handles, delete button). This is the single source of truth for the mode boundary: writing mode + no drawing = elements live, everything else = elements inert.
  ...
  - **Delete pill:** red "Delete" `Capsule` button at the top of the current page (`isCurrent`), shown in **both** modes. Tapping it **always opens a confirmation dialog first** (`.alert("Delete this page?")`, `@State showDeleteConfirmation`); Cancel leaves the page untouched. On **Delete**: carousel mode (`!writingMode`) → `store.requestDeletePage(at: pageIndex)`, the animated ghost + rising-replacement flow (§8); writing mode → `store.removePage(at: pageIndex)` directly (no carousel to animate the exit) followed by `store.exitWritingMode()` so the user lands back on the carousel with the (possibly re-indexed) current page, and `removePage` already cleared selection/focus/draw mode. The pill counter-scales by `1/contentScale` so it stays readable on the downscaled carousel paper.
  - **Tap handling:**
...
- **Purpose:** `UIViewRepresentable` wrapping `PKCanvasView`, used inside the current page's `PageContentView` in both carousel and writing mode. In carousel mode (and in writing mode before drawing is armed) it's a non‑interactive preview of the existing strokes; in writing mode with Scribble armed (`store.drawMode`) it's the live drawing surface with the system tool picker.
- **Key behaviour:**
  - `canvasView.isUserInteractionEnabled = store.writingMode && store.drawMode` — drawing requires BOTH the writing-mode surface AND the explicitly-armed Scribble tool. Entering writing mode alone (pinch-in, Add Text/Image/Audio) leaves the surface inert; tapping the toolbar Scribble tool arms it.
  ...
  - When `(writingMode && drawMode)` flips to true (via `setWritingMode`), the canvas becomes first responder and shows `PKToolPicker`, registered as observer. When it flips to false, the tool picker hides, the canvas resigns, and interaction disables.
...
- Paper geometry tokens/functions: `pageAspectRatio = 1.3`, `writingCanvasSize(for:)` (the canonical content coordinate space, used as `store.canvasSize` in **both** modes and rendered 1:1 by writing mode), `carouselSlotSize(for:)` (the deck display size; the carousel renders canonical content scaled down to fit it). Centralising these keeps carousel and writing papers shape‑identical and content‑region‑identical.
...
| `Features/Journal/Models/Page.swift` | One page: owns its own elements + scribble layer + `createdAt` date (printed on the paper). |
...
Deleting (current page only — Delete pill is only shown when isCurrent; both modes confirm first):

  ANY MODE:
    PageContentView Delete pill tap (top of current page)
    ─► showDeleteConfirmation = true
    ─► .alert("Delete this page?") → [Delete, Cancel]
        ─► Cancel → nothing (page unchanged)

  WRITING MODE (no carousel to animate):
    ─► Delete → store.removePage(at: pageIndex)
          ─► pages.remove(at: index)
          ─► if pages.isEmpty → pages = [Page()]; currentPageIndex = 0
          ─► index fixup of currentPageIndex
          ─► clear selection / focusedTextID / drawMode
        ─► store.exitWritingMode()   (swap back to carousel, clears state)

  CAROUSEL MODE (animated):
    ─► Delete → store.requestDeletePage(at: pageIndex)
...
Carousel mode → enter writing mode
  Four equivalent paths:
    • pinch-in (fingers spreading) on the carousel page ─► store.enterWritingMode()        (drawing OFF)
    • tap Scribble toolbar button                       ─► store.enterWritingMode() then store.enableDrawing()
    • tap any other toolbar tool (Add Text/Image/Audio) ─► JournalView.enterWritingModeIfNeeded()   (drawing OFF)
...
Writing mode → exit writing mode
  Three equivalent paths:
    ...
    • tap Scribble toolbar button while drawing is on   ─► store.disableDrawing()   (stays in writing mode)

Drawing is opt-in (store.writingMode && store.drawMode):
...
Exit / toggle off  ─► setWritingMode(false): toolPicker hidden, PKCanvasView resigns
                       first responder, interaction disabled. If writing mode is exited the
                       carousel returns (elements inert again, gated on `isCurrent && store.writingMode`);
                       if only drawing is toggled off the elements on the writing canvas remain interactive.
```
...
4. **One canonical coordinate space, two presentation sizes.** The board renders at a fixed 1:1 scale — there is no zoom or pan. Element drag, width/height handles, and PencilKit touches all read the `"canvas"` named coordinate space directly. All pages are authored against `store.canvasSize = DesignSystem.writingCanvasSize` (the writing paper) in every mode — the carousel never swaps in a smaller coordinate space. The carousel just *presents* that canonical content downscaled into its deck slot (`carouselSlotSize`), so both modes always show the identical region of the board and element clamps never depend on which mode last reported its paper size.
   5. **Mode boundary is the single gate on element interaction.** The element `ForEach` in `PageContentView` is gated by `.allowsHitTesting(isCurrent && store.writingMode && !store.drawMode)`. In carousel mode (`!writingMode`) no element on the current page is hit-testable — taps, double-taps, drags, and resize handles all fall through to the parent carousel (where the swipe and pinch-in gestures live). In writing mode with drawing off the same elements are fully interactive. **Drawing is a separate, explicit gate that also suppresses elements**: the `PKCanvasView` is interactive only when `store.writingMode && store.drawMode` (with `drawMode` = "Scribble armed", `enableDrawing()`, off by default on entry), and while it's armed the element layer is non-hit-testable — the canvas above-typed content receives every stroke instead of the element underneath. `drawMode` also survives as the carousel's swipe guard (`!store.drawMode`) and the page background-tap guard in `PageContentView`, and is cleared by `select` / `focusText` / `disableDrawing` / `exitWritingMode` / page switches.
...
9. **Min/max constraints.** Elements are clamped to `minBlockWidth × minBlockHeight` and to the canonical `canvasSize` (`DesignSystem.writingCanvasSize`) on resize and drag.
...
13. **Page deletion always confirms and is destructive.** A red "Delete" pill sits at the top of the current page in **both** modes; tapping it always opens the confirmation dialog and Cancel leaves the page untouched. On Delete: carousel mode runs the animated flow (`requestDeletePage` → `confirmPendingDeletion`, which preserves the exact `removePage` semantics); writing mode calls `store.removePage(at:)` directly and then `store.exitWritingMode()` so the user returns to the carousel rather than staying on a deleted page. Either way the store guarantees "at least one page" via its existing fallback.
...
| Delete page            | Tap red "Delete" pill on current `PageContentView` → confirmation `.alert` → **Delete**: carousel runs `store.requestDeletePage(at:)` → animated ghost + entry via `PageCarouselView` → `store.confirmPendingDeletion()` → `store.removePage(at:)`; writing runs `store.removePage(at:)` directly then `store.exitWritingMode()` (back to carousel). **Cancel**: no-op |
| Enter writing mode     | Pinch-in (fingers spreading, `magnification > 1.3`) on the carousel **OR** tap ANY toolbar tool → `store.enterWritingMode()` (only the Scribble tool additionally arms drawing via `store.enableDrawing()`) |
| Exit writing mode      | Pinch-out (fingers closing, `magnification < 0.7`) in `WritingCanvasView` **OR** tap "Exit Writing" toolbar button → `store.exitWritingMode()`; tap Scribble again to merely disarm drawing (`store.disableDrawing()`) |
| Draw / erase           | In writing mode *with Scribble armed* (`store.drawMode`): touch / Pencil on `PKCanvasView` → `store.scribble = canvasView.drawing` |
...
2. **Mode separation (carousel vs writing).** Carousel mode is **browse only** — the `PKCanvasView` is inactive (no strokes), and the element `ForEach` in `PageContentView` is gated `.allowsHitTesting(isCurrent && store.writingMode && !store.drawMode)` so all element gestures (tap, double-tap, drag, resize, delete) are inert. Writing mode is the only place the current page is interactive: elements are hit-testable **only while drawing is off**, and the `PKCanvasView` accepts strokes **only when the Scribble tool is armed** (`store.drawMode`, via the toolbar) — when armed, the element layer is inert so strokes draw over content rather than dragging it. **Entry paths into writing mode:** pinch-in (fingers spreading, `magnification > 1.3`) on the carousel, the Scribble toolbar button (which also arms drawing), OR any of the other three toolbar tools (Add Text / Add Image / Add Audio) — every tool tap in carousel mode calls `JournalView.enterWritingModeIfNeeded()` before performing its action. **Exit paths:** pinch-out (fingers closing, `magnification < 0.7`) in `WritingCanvasView`, the always-present "Exit Writing" toolbar button, or (once drawing is armed) the same Scribble button toggling drawing back off. Two distinct gates — `writingMode` (which surface) and `drawMode` (is drawing engaged) — don't conflate them; entering writing mode must NOT auto-arm drawing.
...
12. **Mode transitions must reset every UI gate the previous mode set.** `enterWritingMode()` leaves `drawMode = false` (drawing is opt-in); `exitWritingMode()` clears both `writingMode` and `drawMode`. Same pattern for any future mode flag: entering a mode is paired with exiting. The carousel's swipe guard (`!store.drawMode`) is what made the omission user‑visible — keep that invariant whenever you add a new mode‑gated state. Also wrap the mode flip in `withAnimation(DesignSystem.modeSwitchAnimation)` (as `enterWritingMode` / `exitWritingMode` / `enableDrawing` / `disableDrawing` all do) so the `JournalView` surface swap stays animated; do this in the store method, not per‑call‑site, so every entry/exit path animates.
...
MiroCloneApp
   └── JournalView
         ...
         ├── PageCarouselView              ── reads/writes store.pages, store.currentPageIndex, store.pendingDeletion; calls store.requestDeletePage (delete animation) / store.confirmPendingDeletion; reports the canonical canvas size (DesignSystem.writingCanvasSize) to store.updateCanvasSize; renders canonical-size page content scaled down into each slot; drives visualPageIndex from drag and during delete animations, animates on commit
         │     └── PageContentView × (pages.count ± 1)
         │           ├── ScribbleCanvasView       (only when isCurrent)  ── reads/writes store.scribble (current page); gates interaction + tool picker on store.writingMode && store.drawMode; boundPageID + lastAssignedDrawingData track identity + echo
         ...
         │           └── deleteButton            ── carousel: store.requestDeletePage(at:) (animated) / writing: confirmation dialog → store.removePage(at:)
...
Read `MiroCloneApp` → it launches a `JournalView`. `JournalView` creates a `CanvasStore` (the brain) and a `NavigationStack` whose body swaps between two views based on `store.writingMode`: `PageCarouselView` (carousel mode) or `WritingCanvasView` (writing mode). The swap is animated — the store flips the flag inside `withAnimation(DesignSystem.modeSwitchAnimation)` and both surfaces carry a 50%-scale + opacity transition, so switching reads as a zoom-in / zoom-out on the current page. The toolbar adds new content (Add‑Text, `PhotosPicker`, Mic → `AudioRecorderSheet`) and arms/controls drawing via the Scribble button: in carousel mode it enters writing mode *and* arms drawing; in writing mode it toggles drawing on and off (with a separate "Exit Writing" button always present). Drawing is opt‑in — entering writing mode by pinch‑in or by adding content never auto‑arms the pencil.

The carousel is the entire board in **carousel mode**, modeled as a deck of pages. Pages are rendered at paper‑like size (≈ 45% of container width), with the current page centered and the previous / next pages pushed far below — only their top 20% peeks above the container bottom edge, reading as the top of a deck beneath the viewport. Side pages are also slightly smaller (`0.95×`) for a hint of perspective. A dynamic page number sits under each page. The rightmost slot, when on the last page, becomes a dashed "Add Page" card. The current page has a red "Delete" pill at the top, which opens a confirmation dialog before anything is removed. Navigation is by swipe, by tapping a side page, by tapping the Add Page card, or by swiping past the last page. Transitions are driven by a single fractional `visualPageIndex` that springs on commit, making the incoming page visibly rise from its lower side position into the center while the outgoing page falls into the side position — a deck‑of‑cards feel rather than a simple horizontal slide. After the user confirms a delete, the carousel runs the same spring: the page drops and slides slightly outward toward the side opposite the replacement while fading and shrinking, and the replacement page rises from its lower side position into center. After `deleteAnimationDuration = 0.5s` the carousel confirms the deletion, which calls `removePage(at:)` under the hood — the existing array‑mutation semantics are preserved exactly. In carousel mode, **drawing is disabled**: the `PKCanvasView` underneath the current page is `isUserInteractionEnabled = false`, so a Pencil/finger stroke on the small page preview does nothing.

In **writing mode**, `JournalView` swaps in `WritingCanvasView`, which renders a single `PageContentView` at writing‑canvas size (≈ 85% of container width × 75% of height at paper aspect 1.3). The page strip and neighbours are hidden; the red **Delete pill** is still shown at the top of the current page. It opens the same confirmation dialog, and confirming deletes via `store.removePage(at:)` (no carousel to animate) **then exits writing mode** (`store.exitWritingMode()`), so the user lands back on the carousel instead of remaining on a deleted page. The `PKCanvasView` is the live drawing surface **only when Scribble is armed** (`store.drawMode`): until then the canvas stays inert and no tool picker appears, so adding text/images/audio or just browsing in writing mode never surprises you with a pencil. Once armed, the canvas becomes first responder with the system `PKToolPicker` visible and the user can write / erase / use any PencilKit tool. Ways out: pinch‑out (which calls `store.exitWritingMode()`, tearing the writing canvas down and returning to the carousel), the always‑visible "Exit Writing" toolbar button, or tapping Scribble again to disarm drawing while staying in writing mode. The writing paper is the **canonical content coordinate space**: `store.canvasSize` holds `DesignSystem.writingCanvasSize` in *both* modes, so clamps never shift between modes and anything authored here is fully visible on the smaller carousel paper (which downscales the same canonical content to fit its slot).

Each `PageContentView` renders one page: a paper background, a scribble layer (interactive `PKCanvasView` for the current page, static `UIImage` snapshot for neighbors), the page's creation date printed at the top-center ("dd MMMM yyyy"), and a `ForEach` of `ElementContainerView`s for each element. `PageContentView` owns its own `"canvas"` coordinate space, so element drag math is always in the page's own coordinate system. Element hit‑testing is gated to `isCurrent && store.writingMode && !store.drawMode`, so neighbouring pages are never interactive, in carousel mode even the current page's elements are inert previews, and in writing mode the elements are fully interactive **only while drawing is disarmed** (tap, double-tap, drag, resize, delete). The `PKCanvasView` beneath them accepts strokes only when Scribble is armed (`store.writingMode && store.drawMode`); when drawing is armed the element layer is inert, so strokes land on the canvas even directly over an element; when only writing mode is active (no drawing), the canvas is inert and touches fall through to element gestures on the same surface.
...
- Writing mode is entered via a `MagnifyGesture` with `magnification > 1.3` (fingers spreading) on `PageCarouselView`, the Scribble toolbar button (which additionally arms drawing), or any of the other three toolbar tools (Add Text / Add Image / Add Audio, each routed through `JournalView.enterWritingModeIfNeeded()`); exited via the "Exit Writing" toolbar button or a `MagnifyGesture` with `magnification < 0.7` (fingers closing) on `WritingCanvasView`. Drawing never auto-arms: only the Scribble toolbar tool (`store.enableDrawing()`) turns the `PKCanvasView` interactive.
...
- Multi‑page behavior: app starts with one empty page, `addPage()` appends, `removePage(at:)` removes (confirmed in writing mode, animated+unconfirmed in carousel) and falls back to an empty page if the last one is deleted, `switchToPage(at:)` clears selection/focus/draw mode.
- Page deletion is animated in the carousel via `requestDeletePage(at:)` (grabs `store.pendingDeletion`) → carousel ghost + rising-replacement animation → `confirmPendingDeletion()` → `removePage(at:)`. Both modes confirm via the Delete pill's `.alert` first; in writing mode the confirmed delete calls `removePage(at:)` directly and then `exitWritingMode()` to return to the carousel.
